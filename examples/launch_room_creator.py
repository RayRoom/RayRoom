...
try:
    ...
    from rayroom.room.objects import Furniture, Source, Receiver, Object3D
except ImportError:
    print("Error: Could not import rayroom. Ensure you are running this from the examples directory or have installed the package.")
    sys.exit(1)
    ...
    for name, cls in inspect.getmembers(objs):
        ...
        if obj_type == 'furniture':
            try:
                ...
                # print(f"  Could not extract geometry for {name}: {e}")
                pass
...
def main():
    ...
    try:
        ...
        template_path = os.path.join(os.path.dirname(__file__), '..', 'rayroom', 'room', 'templates', 'room_creator_template.html')
