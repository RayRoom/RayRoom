...
Reference:
    - Käser & Dumbser (2006) "An arbitrary high-order DG method for elastic waves"
      https://www.geophysik.uni-muenchen.de/~igel/PDF/kaeseretal_gji_2006.pdf
    - Hesthaven & Warburton (2008) "Nodal Discontinuous Galerkin Methods"
      https://github.com/ml14je/DGFEM
    ...
    def precompute_derivatives(self):
        ...
        for i in range(self.num_nodes):
            for j in range(self.num_nodes):
                ...
                node_minus[0] -= h
                self.Dr[i, j] = (self.lagrange_poly(i, node_plus) - 
                ...
                node_minus = self.nodes[j].copy()
                node_minus[1] -= h
                self.Ds[i, j] = (self.lagrange_poly(i, node_plus) - 
                                 self.lagrange_poly(i, node_minus)) / (2 * h)
                ...
                node_minus = self.nodes[j].copy()
                node_minus[2] -= h
                self.Dt[i, j] = (self.lagrange_poly(i, node_plus) - 
                                 self.lagrange_poly(i, node_minus)) / (2 * h)
...
class DGFEMSolver:
    ...
    def __init__(
        ...
        self.elements_connectivity = mesh_elements
        self.config = config or DGFEMConfig()
        self.c = c
        self.rho = rho
        
        # Physics constants
        self.bulk_modulus = rho * c**2
        ...
        print(f"DG-FEM Solver initialized:")
    ...
    def compute_rir(
        ...
        if self.xp == cp:
            rir_dt = cp.asnumpy(rir_dt)  # Transfer from GPU
...
if __name__ == "__main__":
    ...
    vertices, elements = create_box_mesh([1.0, 1.0, 1.0], resolution=0.3)
    ...
    solver = DGFEMSolver(vertices, elements, config)
    
    # Compute RIR
    source = np.array([0.3, 0.3, 0.5])
    receiver = np.array([0.7, 0.7, 0.5])
    
    rir = solver.compute_rir(source, receiver, duration=0.05)
    
    print(f"\nRIR computed: {len(rir)} samples")
